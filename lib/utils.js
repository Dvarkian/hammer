...
export function isIncompatibleModelError(errorText, status) {
  ...
  return /content cannot be a plain string|model does not support text input|only supports (?:the )?interactions api|only supports (?:real[- ]time )?bidirectional streaming(?: via websocket)?|bidiGenerateContent(?: via websocket)?|gemini live api|chat.completions.*(?:is not supported|not available|unsupported)|route not supported|only available on agentic harnesses|calibration|requires terms acceptance|model_terms_required|terms (?:have|has) not been accepted|requires acceptance of the terms|requested model is not supported|model_not_supported/i.test(t)
}
...
export function preferHealthyProviders(ranked, health, now = Date.now()) {
  ...
  const healthy = ranked.filter(r => !isProviderBenched(health, r?.providerKey, now))
