 ...
 *
 * Several providers never report one: some relays set `usage: null` on every stream
 * frame (asking for `stream_options.include_usage` does not help) and answer
 * non-streaming with a bare {content}. A response with no token count has
...
/**
 * The tightest *upper* bound on a row's context window that the router can justify, or null
 * when nothing bounds it from above.
 *
 * Deliberately not `deriveContextState().tokens`: that value is what the dashboard shows, and
 * for a row known only from soft observations it is the *lower* bound ("&gt;6") — a statement
 * about the smallest prompt that got through, not about the largest one that fits. A ceiling
 * has to come from above, so this reads only the two bounds that do: the catalog/provider
 * window, and an observed maximum (which is the size of a prompt the model refused, so the
 * window is at most that). Using a lower bound as a ceiling would clamp a model's answers down
 * to the size of its own prompt.
 */
export function resolveContextUpperBound(knownCtx, stats) {
  const bounds = []
  const known = parseContextSize(knownCtx)
  if (known != null) bounds.push(known)
  const max = Number(stats?.contextMax)
  if (Number.isFinite(max) && max > 0) bounds.push(Math.round(max))
...
// The money half of the account refusals (see isAccountBudgetRefusalText): a stated *balance*
// or *funds* shortfall is a wall rather than a window, because no clock reopens it. The two
// orders are both real — "insufficient balance" / "insufficient credit balance", and
// SiliconFlow's own wording, live 2026-09-23: "Sorry, your account balance is insufficient",
// which it answers a Test with on an HTTP 200 whose body otherwise reads as a completion.
//
// Deliberately about balance and funds and *not* about credits alone: a provider's free-credit
// allowance can be refilled by a quest, so it stays on the clock where it has always been
// instead of becoming a wall this predicate cannot see past.
...
/**
 * Detects payment-required rejections: HTTP 402, error text demanding a paid
 * plan / billing action (e.g. OpenAI's "Payment required...", Ollama cloud's
 * "requires both a Pro, Max, or Team plan ... upgrade for access"), or an account
 * whose own balance or funds are stated to be out (see INSUFFICIENT_FUNDS_RE).
 * Free-tier rate limits ("add 10 credits to unlock") are NOT treated as payment
 * walls.
 */
export function isPaymentRequiredError(errorText, status) {
  ...
  if (INSUFFICIENT_FUNDS_RE.test(t)) return true
  return false
}
 ...
 * the details has to have somewhere to put them. Some providers' validation failures arrive as
 * `{"success":false,"error":{"message":"Something was wrong
 ...
 * whose first frame is an error envelope, such as g4f's
...
/**
 * The verdict of a test response that no window can reopen, or null.
 *
 * Three of the states the router can learn from a response are statements about what the row
 * *is* rather than reports of something that went wrong once: the model is behind money
 * (paid), the provider no longer serves it (dead), or it does not serve text chat at all
 * (incompatible). Nothing about them improves with time, which is what separates them from the
 * waiting states — a rate limit, a quota, an overload — whose whole content is "come back
 * later".
 *
 * They are read from one place because both the base classification and the bench-clearing
 * rule need the same answer about the same message: a row sitting on a rate-limit window while
 * its newest response says "requires an active subscription" was a clock nobody could act on,
 * because the paid verdict could never clear a bench (see resolveModelStatus) and the refresh
 * would re-derive the clock from the flag the bench persisted.
 *
 * @returns {'paid'|'dead'|'incompatible'|null}
 */
export function permanentTestVerdict(lastResponse, now = Date.now()) {
  if (!lastResponse) return null
  // An expired record is no evidence at all, exactly as the base rule reads it — a timeout
  // window that has closed cannot keep condemning a row.
  if (lastResponse.expiresAt != null && now > Number(lastResponse.expiresAt)) return null
  const code = Number(lastResponse.status || 0)
  const errText = String(lastResponse.error || '')
  // Money before the tombstone, and both before the modality: the orders the base rule below
  // has always used, kept here so the two cannot disagree about the same response.
  if (lastResponse.paymentRequired === true || code === 402 || isPaymentRequiredError(errText, code)) return 'paid'
  if (lastResponse.dead === true || code === 410 || isDeadModelError(errText, code)) return 'dead'
  if (lastResponse.incompatible === true || isIncompatibleModelError(errText, code)) return 'incompatible'
  return null
}

/**
 * The verdict a refusal earns, in the one order every path reads it.
 *
 * This is the classification the manual Test has always applied to a refusal it received, named
 * so that a *proxied* failure can be recorded as the same kind of evidence. A request the router
 * sent to a model that then refused is the same statement about that row as a Test click that
 * came back the same way — money, a tombstone, a modality, a window — and a router that shrugged
 * and tried the next model left the row reading healthy, which is how the same model kept being
 * selected for the next turn.
 *
 * The permanent three come first (see permanentTestVerdict): they outrank the waiting states
 * because they are not waits.
 *
 * @returns {'paid'|'dead'|'incompatible'|'rate-limited'|'overloaded'|'noauth'|'timeout'|'down'}
 */
export function resolveRefusalVerdict(errorText, status) {
  const code = Number(status) || 0
  const permanent = permanentTestVerdict({ status: code, error: errorText })
  if (permanent) return permanent
  if (code === 429 || isRateLimitedErrorText(errorText) || isQuotaExhaustionError(errorText, code)) return 'rate-limited'
  if (code === 503 || isProviderOverloadedError(errorText, code)) return 'overloaded'
  if (code === 401 || code === 403) return 'noauth'
  if (code === 408 || code === 504 || /timed?\s*out/i.test(String(errorText || ''))) return 'timeout'
  return 'down'
}

/**
 * Whether a fallback's observability record still describes a valid event, given the rows it
 * was made from. This does not make the fallback a selection rule: routing always re-evaluates
 * manual pin > slope line > ranking. The record is kept only while the replacement is usable
 * and the model it replaced is not, so the dashboard can explain that particular retry without
 * allowing stale evidence to relabel a later selection.
 */
export function isFallbackSelectionActive(selection, rows, isEligible = () => true) {
  if (!selection || !selection.modelId) return false
  const list = Array.isArray(rows) ? rows : []
  const find = (modelId, providerKey) => list.find(r => r && r.modelId === modelId && (!providerKey || r.providerKey === providerKey)) || null
  const usable = (row) => !!row && row.status === 'up' && isEligible(row)
  const row = find(selection.modelId, selection.providerKey)
  const replaced = selection.fromModelId ? find(selection.fromModelId, selection.fromProviderKey) : null
  if (!usable(row) || !replaced) return false
  return !usable(replaced)
}

/**
 * Which of the router's selection rules produced the model it is using, in the order they
 * outrank one another.
 *
 * The dashboard draws its "Current" readout, the slope plot's highlighted dot and the network
 * plot's active pair from this same choice, so naming its source is what lets the page say *why*
 * a model is the current one. Fallback metadata is intentionally not an input: it explains a
 * completed retry, but never participates in the current selection.
 *
 * @returns {'pin'|'slope'|'ranked'|null}
 */
export function resolveSelectionSource({ pinned = null, slope = null, ranked = null } = {}) {
...
export function resolveModelStatus(row, now = Date.now()) {
  ...
  // The newest response's statement that no window can reopen this row (see
  // permanentTestVerdict), read once for the bench rules below and the base rule at the end.
  const permanent = permanentTestVerdict(lr, now)
  ...
  if (isRateLimited) {
    ...
    // A newer permanent verdict retires the bench exactly as a newer success does. The bench
    // claims the row comes back when its window ends; the response says waiting will not help,
    // and the money, the roster or the modality it names is what the user has to act on.
    // Ordering is what keeps this honest — a 429 observed *after* that response is the newer
    // story and keeps its clock (capturedAt is the bench's own timestamp).
    const testCleared = (isLastResponseReady(lr, now) || permanent != null) && testAt > rateLimitCapturedAt
  ...
  if (rawStatus === 'overloaded') {
    ...
    const testCleared = (isLastResponseReady(lr, now) || permanent != null) && testAt > overloadAt
  ...
  if (lr) {
    ...
    if (!expired) {
      ...
      // Money, a tombstone, or a model that does not serve chat: the same three verdicts the
      // bench rules above read, from the one place that decides them.
      if (permanent) return permanent
  ...
  //
  // A permanent verdict a probe or the proxy recorded on `status` (with its message on
  // `lastError`) is honored here, because those paths do not write a lastResponse at all: a
  // model a ping found behind a paywall has no test evidence for the rules above to read, and
  // dropping the probe's own verdict is what left such a row reading Down. banned, excluded,
  // noauth and overloaded are trusted the same way, for the same reason. Their subject is a
  // *refusal*, not a healthy row: a 200 has somewhere else to live (a ready test response, or
  // real traffic), and a liveness ping alone still does not make a row Up here. Any success — a
  // passing Test, or a ping that answered — writes `status: 'up'`, which is what clears them.
  if (rawStatus === 'paid' || rawStatus === 'dead' || rawStatus === 'incompatible') return rawStatus
...
// (some providers) and "insufficient credit balance" / "you are out of credits".
//
...
// complete a quest, then try again." — a wording some providers use once the key's credit
// balance is spent. Neither of the first two shapes sees it
...
export const MAX_REFUSAL_NOTICE_CHARS = 600
 ...
 * Some gateways, for example, answer HTTP 200 with "The API key used for this request has
 ...
 * Some gateways serve one without debiting the account — for example with `x-cache:
 * HIT` with a year-long immutable `cache-control` on a key whose every uncached request was
 * refused with an account-credit error. A hit therefore
...
// How providers spell the output budget in a refusal, as one fragment so every rule below
// reads the same set of names: chat-completions' `max_tokens`, the newer
// `max_completion_tokens`, Google's camelCase `maxOutputTokens`, and the prose providers put
// around the same thing — Cohere's "max tokens", Ollama's "maximum output tokens",
// "output length". Deliberately not a bare "max" or "limit", and deliberately not
// "context": a context statement is parseContextLimitFromError's business.
// Kept as one non-capturing group, because the rules below concatenate it with the rest of
// the pattern and a bare alternation would bind only to its last branch.
const OUTPUT_BUDGET_NOUN = String.raw`(?:max[\s_-]*(?:output[\s_-]*|completion[\s_-]*)?(?:tokens?|length)|maxOutputTokens|maxCompletionTokens|(?:maximum[\s_-]+|max[\s_-]+)?output[\s_-]*(?:tokens?|length)|completion[\s_-]*tokens?)`

// The budget the caller asked for, written right after the noun — Ollama's
// "max_tokens (393216) exceeds …". Skipped so no rule can ever read the requested number as
// the ceiling (that would clamp to a number that changes nothing).
const REQUESTED_BUDGET_INFIX = String.raw`(?:\s*[:=]?\s*\(?\s*[\d,.]+\s*\)?)?`

/**
 * Extracts a provider-stated output ceiling for a model, e.g.
 * "`max_tokens` must be less than or equal to `8192`, the maximum value for
 * `max_tokens` is less than the `context_window` for this model" (Groq) -> 8192,
 * "too many tokens: max tokens must be less than or equal to 4096, the maximum output
 * length for this model - received 393216." (Cohere) -> 4096, or
 * "max_tokens (393216) exceeds model's maximum output tokens (131072)" (Ollama) -> 131072.
 *
 * Every rule pairs a *budget noun* with a stated ceiling, so a body that only compares the
 * request against the model's context window (OpenRouter's "maximum context length is
 * 262144 tokens … 393216 in the output") yields nothing here — that number is
 * parseContextLimitFromError's, and reading it as an output cap would be a different fact
 * about the model.
 *
 * A throughput quota is refused outright: Groq's "Request too large … on output tokens per
 * minute (OTPM): Limit 1000, Requested 1413" names an output budget and a number that is not
 * a ceiling, and learning 1000 there would trim every answer to that model to 1000 tokens
 * for good. Same call the context parser makes (see isRateLimitBodyText).
 *
 * Used to retry a request (or a Test) with a legal max_tokens value; a wrong number is
 * expensive, so every rule below is deliberately strict and a miss just means no clamp.
 */
export function parseMaxTokensCapFromError(errorText) {
  const t = String(errorText || '')
  if (!t.trim()) return null
  if (isRateLimitBodyText(t)) return null
  const patterns = [
    ...
    // "<budget> must be less than or equal to <ceiling>" — Groq's backticked `max_tokens`
    // form, Cohere's prose "max tokens must be less than or equal to 4096", and the
    // "must be <= 4096" / "Invalid max_tokens" variants.
    new RegExp(`${OUTPUT_BUDGET_NOUN}${REQUESTED_BUDGET_INFIX}[^\\d\\n]{0,60}?(?:must\\s+be\\s+)?(?:less\\s+than\\s+or\\s+equal\\s+to|no\\s+more\\s+than|not\\s+more\\s+than|no\\s+greater\\s+than|at\\s+most|up\\s+to|<=|≤)[^\\d\\n]{0,12}([\\d,.]+)`, 'i'),
    // "… exceeds the maximum…", whose ceiling is named after the verb and never the number
    // that was asked for. Both real shapes: Ollama's "max_tokens (393216) exceeds model's
    // maximum output tokens (131072)" and "max_tokens: 393216 is greater than the maximum
    // allowed 4096".
    new RegExp(`${OUTPUT_BUDGET_NOUN}${REQUESTED_BUDGET_INFIX}[^\\d\\n]{0,30}?(?:exceeds?|is\\s+greater\\s+than|is\\s+larger\\s+than|is\\s+over|goes\\s+over|is\\s+above)\\b[\\s\\S]{0,140}?(?:max(?:imum)?|limit|ceiling|allowed|cap)[^\\d\\n]{0,40}([\\d,.]+)`, 'i'),
    // "<budget> is/of <ceiling>", e.g. "maximum output tokens for this model is 4096".
    new RegExp(`${OUTPUT_BUDGET_NOUN}[^\\d\\n]{0,24}?\\b(?:is|of|are|equals?)\\b\\s*\\(?([\\d,.]+)`, 'i'),
  ]
...
/**
 * The floor a context-derived ceiling is never taken below. A prompt that already fills the
 * window leaves no room at all, and "ask for zero tokens" is not a request any provider can
 * answer; that request is over-length, which the caller treats as a failover instead.
 */
export const MIN_CLAMPED_OUTPUT_BUDGET = 256

/**
 * The tightest output budget a row can actually accept, from facts the router already holds:
 * a ceiling the provider has stated for this model, and/or the model's context window less
 * what the prompt already occupies.
 *
 * A caller that states a budget no model can emit — Freebuff's fixed 393216, for one —
 * otherwise spends its first attempt on a refusal that was predictable from the window the
 * dashboard was showing all along, and on a strict provider that first attempt is the whole
 * turn.
 *
 * Only known bounds are used: a row with neither a stated ceiling nor a context window is
 * left alone rather than given an invented one. A context-derived ceiling can only lower a
 * budget, never raise it (see withOutputBudgetCap), so the worst case is an answer shorter
 * than the caller hoped for — never a request the provider refuses on our account.
 *
 * @param {object} bounds
 * @param {number|null} bounds.learnedCap     ceiling the provider has stated for this row
 * @param {number|null} bounds.contextTokens  the row's context window, in tokens
 * @param {number|null} bounds.promptTokens   what this prompt already occupies
 * @param {number} [bounds.floor]             floor for the context-derived part
 * @returns {number|null} ceiling to clamp to, or null when the row states none
 */
export function resolveOutputBudgetCap({ learnedCap = null, contextTokens = null, promptTokens = null, floor = MIN_CLAMPED_OUTPUT_BUDGET } = {}) {
  const minimum = Number(floor) > 0 ? Math.floor(Number(floor)) : 1
  const caps = []
  const learned = Math.floor(Number(learnedCap))
  if (Number.isFinite(learned) && learned > 0) caps.push(learned)
  const context = Number(contextTokens)
  if (Number.isFinite(context) && context > 0) {
    const prompt = Number(promptTokens)
    const room = Math.floor(context - (Number.isFinite(prompt) && prompt > 0 ? prompt : 0))
    caps.push(Math.max(minimum, room))
  }
 ...
 * and resolveOutputBudgetCap for which bounds may be used at all.
 *
 ...
 *
 * 2: the budget noun is read as providers actually write it ("max tokens", "maximum output
 * tokens", "maxOutputTokens", "output length"), not only as `max_tokens`; and a body that
 * states a throughput quota can no longer become a ceiling at all. Every stored cap is
 * re-examined against these rules at startup (see revalidateOutputCap), so a number the old
 * rules misread is withdrawn rather than trimming that model's answers forever.
 */
export const OUTPUT_CAP_PARSER_VERSION = 2
...
/**
 * True when a refusal says the *payload we sent* is too large for this model — the context it
 * cannot hold, or the output budget it cannot emit — whether or not the provider stated a
 * number.
 *
 * This is the failover-facing reading of the family, and it is deliberately not
 * isOverLengthErrorText: that predicate still decides whether the prompt size we happened to
 * send may become a learned ceiling for the row (see recordContextObservation), so widening
 * *it* is how Groq's 131k-token rows were struck out of the table over a 1000-token
 * throughput quota. A refusal the router cannot make legal for this model — a prompt that
 * already fills the window, a budget nothing can raise — should move to a model that can
 * serve it instead of ending the turn with the provider's 400.
 *
 * A model that cannot support chat at all ("does not support chat endpoints") is a statement
 * about the model rather than about our payload, and stays out of this.
 */
export function isRequestTooLargeErrorText(errorText) {
  const t = String(errorText || '')
  if (!t.trim()) return false
  if (parseMaxTokensCapFromError(t) != null) return true
  if (isOverLengthErrorText(t)) return true
  // Providers that state no number still say what to do about it. Groq, for one:
  // "Please reduce the length of the messages or completion."
  if (/\breduce\s+the\s+length\b/i.test(t)) return true
  if (/\b(?:payload|request|input|prompt|messages|completion)\b[^.\n]{0,24}\btoo\s+(?:large|long)\b/i.test(t)) return true
  return false
}
 ...
 *   search      — search models, which are provider tools rather than general chat models.
 *
...
export function isBlockedModelName(modelId) {
  return /calibration|saudi|audio|translat|search/i.test(String(modelId || ''))
}
 ...
 * hosted by g4f land under the manufacturer's row.
 */
