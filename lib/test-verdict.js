...
export function summarizeTestAnswer(transcript) {
  ...
  const answer = {
    ...
    streamed: false,
    parsed: false,
    // Why the provider stopped, when it said so. 'length' means the answer was cut off by the
    // output budget, which is a statement about the budget the caller chose rather than about the
    // model — the test retries once with more headroom on it (see the headroom retry in
    // lib/server.js) instead of reporting a model that returned no text.
    finishReason: null,
  };
