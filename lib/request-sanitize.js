...
import { scrubContentResidue } from './tool-call-residue.js'
...
/**
 * Removes DeepSeek tool-call envelope residue from every message's content.
 *
 * `stripAssistantReasoning` can be scoped to assistant turns because only a model writes reasoning
 * for itself. Envelope residue does not stay in the turn that produced it: a client renders the
 * assistant's output into its own progress notes and conversation summaries, and the observed
 * transcript carried the fragment inside a *user* turn ("Progress note: …") and inside a
 * `<conversation_summary>` block. Every message is walked, whatever its role.
 *
 * This is the half that stops the leak propagating. The proxy also strips the tags on the way out
 * (see lib/tool-call-residue.js), but a transcript that already carries them keeps sending them
 * back until something removes them here.
 *
 * @param {object} body  outbound chat-completions body
 * @returns {{ body: object, stripped: string[] }} body plus the field names removed
 */
export function stripResidueFromHistory(body) {
  if (!isPlainObject(body) || !Array.isArray(body.messages)) return { body, stripped: [] }
  let modified = false
  const messages = body.messages.map(message => {
    if (!isPlainObject(message) || !('content' in message)) return message
    const { content, changed } = scrubContentResidue(message.content)
    if (!changed) return message
    modified = true
    return { ...message, content }
  })
  // Named like `content[thinking]` above: the action is on one nested part of `content`, and the
  // disclosure header has to say which part rather than implying the whole message was rewritten.
  return { body: modified ? { ...body, messages } : body, stripped: modified ? ['content[dsml]'] : [] }
}
...
export function sanitizeProviderPayload(body, providerKey, learned, extraRejection = null) {
  ...
  const residue = stripResidueFromHistory(current)
  current = residue.body
  for (const field of residue.stripped) stripped.add(field)
