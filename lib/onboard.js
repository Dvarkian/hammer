...
// Every g4f relay authenticates with the same g4f.dev credential, so only the
// shared `g4f` entry is prompted for; the relays just point at it.
const G4F_RELAY_KEYS = new Set(['g4f-nvidia', 'g4f-groq', 'g4f-gemini', 'g4f-ollama', 'g4f-pollinations'])
...
export async function runOnboard(port = 7352) {
  ...
  for (const providerKey of PROVIDER_ORDER) {
    ...
    if (G4F_RELAY_KEYS.has(providerKey)) {
      console.log('  note: shares the g4f key; configure it once under `g4f`')
      console.log('  key: (shared)')
      console.log('')
      continue
    }
    if (providerKey === 'g4f') {
      console.log('  note: key is optional. Anonymous use works after baking proof-of-work credits at https://g4f.dev/chat; a free key from https://g4f.dev/members.html unlocks every g4f relay')
    }
