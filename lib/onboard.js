...
// Providers that work with no credential at all. Onboarding records them as
// enabled and skips the key prompt entirely.
const KEYLESS_PROVIDERS = new Set(['freemodels', 'gptfree'])
...
function configureOpenClaw(port) {
  ...
  config.models.providers.hammer = buildOpenClawProviderConfig(port)

  if (!config.agents) config.agents = {}
  if (!config.agents.defaults) config.agents.defaults = {}
  if (!config.agents.defaults.model) config.agents.defaults.model = {}
  config.agents.defaults.model.primary = 'hammer/best'

  if (!config.agents.defaults.models) config.agents.defaults.models = {}
  if (!config.agents.defaults.models['hammer/best']) {
    config.agents.defaults.models['hammer/best'] = {}
  }
...
async function configurePicoClaw(port) {
  ...
  // Add to model_list, replacing any existing hammer entry
  if (!config.model_list) config.model_list = [];
  const existingIdx = config.model_list.findIndex(m => m.model_name === 'hammer');
  if (existingIdx !== -1) {
  ...
  if (!config.agents) config.agents = {};
  if (!config.agents.defaults) config.agents.defaults = {};
  if (!config.agents.defaults.model_name || config.agents.defaults.model_name === '') config.agents.defaults.model_name = 'hammer';
...
export async function runOnboard(port = 7352) {
  ...
  for (const providerKey of PROVIDER_ORDER) {
    ...
    // OAuth providers keep their credential outside apiKeys, so this prompt has to read
    // and write where each provider's own resolver looks, or a token typed here would
    // be saved somewhere nothing reads it.
    const providerConfig = config.providers?.[providerKey]
    const existing = providerKey === 'kiro'
      ? (providerConfig?.refreshToken || '')
      : providerKey === 'openai-codex'
        ? (providerConfig?.accounts?.[0]?.secret || providerConfig?.refreshToken || '')
    ...
    if (KEYLESS_PROVIDERS.has(providerKey)) {
      console.log('  note: no API key is required; this provider is enabled by default')
    ...
    if (providerKey === 'g4f') {
      console.log('  note: key is required — get a free one at https://g4f.dev/members.html')
    }
    ...
    if (providerKey === 'github-copilot') {
      console.log('  note: easiest is the dashboard sign-in (Settings -> GitHub Copilot -> Sign in with GitHub);')
      console.log('        this prompt takes a GitHub OAuth token instead ("gho_...")')
    }
    if (providerKey === 'openai-codex') {
      console.log('  note: easiest is the dashboard sign-in (Settings -> OpenAI Codex -> Sign in with ChatGPT);')
      console.log('        this prompt takes a ChatGPT OAuth refresh token instead')
    }
    ...
    const promptLabel = (providerKey === 'kiro' || providerKey === 'openai-codex') ? '  refresh token: ' : '  key: '
    ...
    if (value === '-') {
      ...
      } else if (providerKey === 'openai-codex') {
        if (!config.providers) config.providers = {}
        if (!config.providers[providerKey]) config.providers[providerKey] = {}
        delete config.providers[providerKey].accounts
        delete config.providers[providerKey].refreshToken
        delete config.providers[providerKey].authEmail
        delete config.providers[providerKey].authMode
        delete config.apiKeys[providerKey]
    ...
    if (value) {
      if (providerKey === 'kiro') {
        ...
        // Codex exchanges this refresh token for its bearer, so it belongs in the
        // account pool the resolver reads, never in apiKeys.
        if (!config.providers) config.providers = {}
        if (!config.providers[providerKey]) config.providers[providerKey] = {}
        config.providers[providerKey].accounts = [{ secret: value, addedAt: Date.now() }]
        delete config.providers[providerKey].refreshToken
        delete config.apiKeys[providerKey]
      } else {
  ...
  saveConfig(config)
  console.log('Saved API keys to ~/.hammer.json')
  ...
  if (multiAccountProviders.length > 0) {
    ...
    if (hasSingleKeys) {
      console.log('\nTip: Add multiple API keys and each is spent before the next is touched, so one account running out is not the end of the road:')
      console.log('  hammer config add-key <provider> <key2>')
      console.log('  hammer config set-maxturns <provider> 20')
    }
  }
