...
/**
 * Live-usage endpoints for providers whose resolution does not carry one.
 *
 * This is the same hook an import uses (`descriptor.usageUrl`), filled in here for the
 * services hammer knows publish an account-usage surface of its own. Only endpoints whose
 * exact shape has been read are listed: a guessed URL is worse than an honest "not
 * reported", because it turns a provider with no quota box into a provider with a wrong one.
 *
 *   - Pollinations documents `GET /account/balance` (gen.pollinations.ai), which answers the
 *     remaining Pollen for the calling key as `{ balance }` — a credit balance, not a token
 *     cap. Its plan is credit-based, so a bar can only ever show what is left.
 */
export const PROVIDER_USAGE_URLS = {
  pollinations: 'https://gen.pollinations.ai/account/balance',
}
...
export const PROVIDER_DESCRIPTORS = {
  ...
  freemodels: {
    ...
    continuity: 'local-transcript',
    toolSupport: 'best-effort',
