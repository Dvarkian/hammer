...
import { NLPCLOUD_DEFAULT_CHAT_URL } from './nlpcloud.js'
...
export const PROVIDER_QUOTAS = {
  ...
  nlpcloud: {
    source: 'Free-plan throughput is provider-managed; NLP Cloud publishes no numeric monthly token allowance',
    sourceUrl: 'https://nlpcloud.com/pricing.html',
  },
}
...
export const PROVIDER_DESCRIPTORS = {
  ...
  // --- NLP Cloud ------------------------------------------------------------
  // NLP Cloud is not OpenAI-compatible: the model and task are URL segments,
  // authentication uses `Authorization: Token`, and the chatbot body is
  // `{ input, context, history }`. It has no model-list endpoint, so the current
  // generative roster is curated in sources.js. The free plan is a recurring
  // $0 account, but its terms restrict it to development and testing.
  nlpcloud: {
    label: 'NLP Cloud',
    chatUrl: NLPCLOUD_DEFAULT_CHAT_URL,
    contextUrl: 'https://docs.nlpcloud.com/#chatbot-and-conversational-ai',
    signupUrl: 'https://nlpcloud.com/home/token',
    auth: {
      envVar: 'NLPCLOUD_API_KEY',
      baseUrlEnvVar: 'NLPCLOUD_BASE_URL',
      scheme: 'Token',
    },
    access: 'permanent',
    tos: 'caution',
    toolSupport: 'unsupported',
    notes: 'The recurring free plan is limited to development and testing; NLP Cloud publishes no numeric monthly token allowance.',
  },
  gptfree: {
    label: 'GPTFree',
