 ...
 * content flattening, and Pollinations' conditional `jsonMode`.
 *
...
export function applyRequestShaping(providerKey, body, modelId = null) {
  ...
  if (shaping.jsonMode) {
    // Only when the caller actually asked for JSON. Pollinations reads `jsonMode` as "must
    // return JSON" and rejects any request whose messages don't mention it, so setting it
    // unconditionally would break every non-JSON request.
    const requested = next.response_format?.type
    if (requested === 'json_object' || requested === 'json_schema') next.jsonMode = true
  }
...
/**
 * A model id that cannot be served without a key, for a provider whose keyless tier is a
 * subset of its catalog.
 *
 * @returns {boolean}
 */
export function shapedModelNeedsKey(providerKey, modelId) {
  const premium = providerShaping(providerKey)?.premiumModels
  if (!Array.isArray(premium) || !modelId) return false
  return premium.includes(String(modelId))
}
