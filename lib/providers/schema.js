...
/** How a provider carries conversation state between requests. */
export const CONTINUITY_MODES = ['request', 'local-transcript', 'stateful']
/** How completely a provider supports OpenAI tool calling. */
export const TOOL_SUPPORT_MODES = ['native', 'best-effort', 'unsupported']
...
export const DESCRIPTOR_DEFAULTS = Object.freeze({
  ...
  quota: null,
  /**
   * Where this provider publishes live account usage, when it has such an endpoint.
   * `null` for the many providers that only expose limits in response headers. A config
   * `usageUrl` overrides this, so an import can point hammer at a provider's own surface
  ...
  discoverable: false,
  /** Whether the provider retains state itself or only sees the request transcript. */
  continuity: 'request',
  /** Whether native OpenAI tool calls are expected to survive the provider adapter. */
  toolSupport: 'native',
...
/** The periods a published limit can be measured over. */
/** The periods a recorded window can be compared against (see `lib/usage-windows.js`). */
export const QUOTA_PERIODS = ['day', 'week', 'month', 'lifetime']

/**
 * One published limit, with the period it applies to.
 *
 * A limit without a period cannot be compared to anything: 15,000,000 tokens against
 * this month's recorded usage and against all of it are different claims. The period is
 * therefore part of the record, not a label the dashboard invents when it draws the bar.
 */
function normalizeQuotaLimit(entry, fallback = {}) {
  if (!entry || typeof entry !== 'object') return null
  const limit = Number(entry.limit)
  if (!Number.isFinite(limit) || limit <= 0) return null
  return {
    metric: ['tokens', 'requests', 'credits'].includes(entry.metric) ? entry.metric : 'tokens',
    limit,
    // The period travels as written. An unrecognized one (`rolling`, `hour`, a provider's own
    // wording) is deliberately not coerced to `month`: the card pairs a limit with the recorded
    // window it names, and a defaulted period would draw a rolling limit against this month's
    // spend — a comparison neither the provider nor hammer ever made. Such a record keeps its
    // number and gets no bar.
    period: asString(entry.period) || 'month',
    label: asString(entry.label),
    source: asString(entry.source) || asString(fallback.source),
    sourceUrl: asString(entry.sourceUrl) || asString(fallback.sourceUrl),
  }
}

function normalizeQuota(quota) {
  if (!quota || typeof quota !== 'object') return null
  const source = asString(quota.source)
  const sourceUrl = asString(quota.sourceUrl)
  // The provenance of the *number* is not always the provider's own documentation: hammer's
  // rows cite the provider's pricing page, while the figure itself can come from the vendored
  // roster. Keeping both means a bar can say where the limit was published without moving the
  // link that answers "where do I read about this plan?".
  const numericSource = asString(quota.numericSource)
  const limitSource = numericSource || source
  const steadyTokensPerMonth = Number.isFinite(quota.steadyTokensPerMonth) ? quota.steadyTokensPerMonth : null
  const signupCreditTokens = Number.isFinite(quota.signupCreditTokens) ? quota.signupCreditTokens : null

  // The flat fields predate the period-aware list and are still what several callers read,
  // so they stay; the list is derived from them and from any explicit entry the descriptor
  // declares. Both shapes describe the same grant, which is why the derived entries are
  // dropped when an explicit one already covers the same metric and period.
  const limits = []
  if (steadyTokensPerMonth != null && steadyTokensPerMonth > 0) {
    limits.push({ metric: 'tokens', limit: steadyTokensPerMonth, period: 'month', label: 'steady free-tier grant', source: limitSource, sourceUrl })
  }
  if (signupCreditTokens != null && signupCreditTokens > 0) {
    limits.push({ metric: 'tokens', limit: signupCreditTokens, period: 'lifetime', label: 'one-time signup credit', source: limitSource, sourceUrl })
  }
  for (const entry of Array.isArray(quota.limits) ? quota.limits : []) {
    const normalized = normalizeQuotaLimit(entry, { source: limitSource, sourceUrl })
    if (!normalized) continue
    if (limits.some(limit => limit.metric === normalized.metric && limit.period === normalized.period)) continue
  ...
  return {
    source,
    sourceUrl,
    /** Where the numeric limit itself was published, when that is not `source`. */
    numericSource,
    ...
    steadyTokensPerMonth,
    /** One-time signup credit, when the catalog publishes a figure. */
    signupCreditTokens,
    /** Every published limit with the period it applies to (see {@link QUOTA_PERIODS}). */
    limits,
    /**
     * Hosts the published grant applies to, when it does not apply to the provider as a
     * whole. Ollama is the reason this exists: one provider key serves a local server bound
     * by your hardware and the hosted `ollama.com` service bound by a monthly grant, so a
     * number taken from the hosted plan must not be shown against local traffic. Empty when
     * the record applies wherever the provider points.
     */
    appliesToHosts: Array.isArray(quota.appliesToHosts)
      ? quota.appliesToHosts.map(host => asString(host)).filter(Boolean)
      : [],
  }
}

/**
 * Whether a published quota record applies to the endpoint a provider is pointed at.
 *
 * A record with no `appliesToHosts` applies wherever the provider points, which is every
 * provider but Ollama: one key there serves a local server bound by your hardware and the
 * hosted `ollama.com` service bound by a monthly grant. Reading the hosted number against
 * local traffic would invent a limit, so the match is checked before the card shows it.
 *
 * @param {object|null} quota a normalized quota record
 * @param {string|null} baseUrl the endpoint the provider will actually be called on
 * @returns {boolean}
 */
export function quotaAppliesToHost(quota, baseUrl) {
  const hosts = Array.isArray(quota?.appliesToHosts) ? quota.appliesToHosts : []
...
export function normalizeDescriptor(key, input = {}) {
  ...
  // These fields are safety claims, not free-form provider metadata: silently
  // turning an unknown value into `native` would make a malformed catalog look
  // healthier than it is. Leave an explicitly supplied invalid value visible to
  // validateDescriptor; only omission/null receives the default.
  const continuity = raw.continuity == null ? DESCRIPTOR_DEFAULTS.continuity : raw.continuity
  const toolSupport = raw.toolSupport == null ? DESCRIPTOR_DEFAULTS.toolSupport : raw.toolSupport
  ...
  return {
    ...
    usageUrl: asString(raw.usageUrl),
    sanitize: {
...
export function validateDescriptor(descriptor) {
  ...
  if (!CONTINUITY_MODES.includes(d.continuity)) problems.push(`invalid continuity "${d.continuity}"`)
  if (!TOOL_SUPPORT_MODES.includes(d.toolSupport)) problems.push(`invalid toolSupport "${d.toolSupport}"`)
