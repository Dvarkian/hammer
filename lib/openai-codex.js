...
export async function transformCodexResponse(response, modelId, stream = false) {
  ...
  // The subscription backend answers 200 with its SSE stream and NO Content-Type
  // header (verified live 2026-09-16), so the header alone cannot pick the branch.
  // When it is missing, the body decides: parseable JSON maps directly, and SSE
  // text falls through to the stream pipeline. Before this, the whole Responses
  // stream passed through raw — the manual test then stored the verbatim
  // 'event: response.created …' transcript as the model's reply.
  const headerSaysEventStream = contentType.includes('text/event-stream');
  const isSseBody = (text) => /^(event|data|id|retry):/m.test(text);

  // A JSON body (some failures, and older backends) is mapped directly.
  if (!headerSaysEventStream) {
    const raw = await response.text();
    let payload = null;
    try { payload = JSON.parse(raw); } catch { /* not JSON */ }
    if (!payload) {
      // No content-type and the body is not JSON: it is the headerless SSE
      // stream. Re-wrap it so it flows through the same pipeline the
      // event-stream branch uses, instead of leaking the raw Responses wire
      // to the client.
      if (isSseBody(raw)) {
        const sseResponse = new Response(raw, {
          status: response.status,
          statusText: response.statusText,
          headers: { 'Content-Type': 'text/event-stream' },
        });
        return transformCodexResponse(sseResponse, modelId, stream);
      }
      return new Response(raw, { status: response.status, statusText: response.statusText, headers: { 'Content-Type': 'application/json' } });
    }
