...
export function buildOpenRouterQualityIndex(catalog, popularityCatalog = catalog, nowMs = Date.now()) {
  ...
  for (const model of models) {
    ...
    if (!entry) {
      ...
      entry = {
        ...
        source: 'metadata',
        isEstimated: true,
        aa: aaForPercentile(score, aaValues),
  ...
  return { index, regression, aaValues, catalogSize: models.length };
}
...
export function predictAAFromElo(elo, regression) {
  ...
  const predicted = regression.intercept + (regression.slope * value);
 ...
 * both, and flagged as an estimate. Metadata and offline fallbacks land on the same
 * AA scale by percentile, so every row shows one comparable number.
 *
 ...
 * is smarter. Estimates keep their own 0-1 score (a heuristic, or scores.js) and
 * take their `aa` from it by percentile, which is the same curve in the other
 * direction.
 */
export function resolveModelQuality(qualityData, modelId, localScore = null) {
  ...
  if (Number.isFinite(normalized) && normalized > 0) {
    ...
    return {
      score,
      aa: aaForPercentile(score, qualityData?.aaValues),
      aaEstimated: true,
      source: 'local-fallback',
      isEstimated: true,
      detail: 'scores.js offline fallback',
      modelId,
    };
  }
