...
import { fetchJsonWithRetry } from './providers/discovery.js';
...
// The live pages are large Next.js shells. LMArena's official dataset exposes the same
// current leaderboards as small, stable JSON and is the normal path; the HTML URLs above
// remain a compatibility fallback for a deployment where the dataset service is blocked.
export const LMARENA_DATASET_URLS = {
  coding: 'https://datasets-server.huggingface.co/rows?dataset=lmarena-ai%2Fleaderboard-dataset&config=webdev&split=latest&offset=0&length=100',
  overall: 'https://datasets-server.huggingface.co/rows?dataset=lmarena-ai%2Fleaderboard-dataset&config=text_style_control&split=latest&offset=0&length=100',
};
export const MODEL_QUALITY_CACHE_MS = 24 * 60 * 60_000;
// The final value is deliberately an AA-scale number, not a normalized 0-1 score:
// an unresolved model must still have a visible place in the ranking. The normalized
// `score` is the same value passed through scoreForAa(), while `aa` remains 0.1.
export const MODEL_QUALITY_FLOOR_AA = 0.1;
...
async function fetchLMArenaHtmlBoard(fetchImpl, url) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 20_000);
  let response;
  try {
    response = await fetchImpl(url, {
      headers: { Accept: 'text/html', 'User-Agent': 'hammer/1.24 (+https://github.com/hammer-app/hammer)' },
...
async function fetchLMArenaDatasetBoard(fetchImpl, url) {
  ...
  if (!Array.isArray(payload?.rows) || payload.rows.length === 0) {
    throw new Error('LMArena dataset returned no leaderboard rows');
  }
  const entries = payload.rows
    .map(item => item?.row)
    .filter(row => row && typeof row.model_name === 'string' && Number.isFinite(Number(row.rating)))
    .map(row => ({
      displayName: row.model_name,
      elo: Number(row.rating),
      votes: Number(row.vote_count) || 0,
    }));
  if (!entries.length) throw new Error('LMArena dataset returned no usable leaderboard rows');
  return entries;
}

async function fetchLMArenaBoard(fetchImpl, { datasetUrl, htmlUrl }) {
  try {
    return await fetchLMArenaDatasetBoard(fetchImpl, datasetUrl);
  } catch (datasetError) {
    try {
      return await fetchLMArenaHtmlBoard(fetchImpl, htmlUrl);
    } catch (htmlError) {
      const datasetMessage = datasetError?.message || datasetError;
      const htmlMessage = htmlError?.message || htmlError;
      throw new Error(`LMArena dataset: ${datasetMessage}; page: ${htmlMessage}`);
    }
  }
}
...
export async function fetchLMArenaBoards({ fetchImpl = fetch, nowMs = Date.now(), force = false } = {}) {
  ...
  const [coding, overall] = await Promise.all([
    fetchLMArenaBoard(fetchImpl, {
      datasetUrl: LMARENA_DATASET_URLS.coding,
      htmlUrl: LMARENA_BOARD_URLS.coding,
    }).catch(err => {
      console.warn('LMArena coding board unavailable:', err?.message || err);
      return [];
    }),
    fetchLMArenaBoard(fetchImpl, {
      datasetUrl: LMARENA_DATASET_URLS.overall,
      htmlUrl: LMARENA_BOARD_URLS.overall,
    }).catch(err => {
...
/**
 * A deliberately narrow family key for the last-resort estimate. It removes
 * provider/routing spelling, runtime tags, dates, and the generic `latest` alias,
 * but keeps model family, variant, and size tokens. This prevents e.g. a 3B model
 * from inheriting an 8B sibling's score while still recognizing common spellings
 * such as `ministral-3:3b` and `ministral-3b-2512`.
 */
export function modelFamilyKey(modelId) {
  let value = String(modelId || '').trim().toLowerCase();
  if (!value) return '';
  value = resolveAliasedModelId(value);
  value = value.replace(/^srv_[a-z0-9]+:/i, '').replace(/^models\//, '');
  value = value.replace(/(?::(?:free|optimized|cloud))+$/i, '').replace(/-(?:free|latest)$/i, '');
  value = value.replace(/^[^/]+\//, '');
  // Normalize the Ollama `model:size` spelling and common provider separators.
  value = value.replace(/(\d+):(\d+b)/g, '$2');
  value = value.replace(/[-_/.:]+/g, ' ');
  return value
    .replace(/\b(?:free|latest|current|default|instruct|it|chat|preview|q\d(?:_\d)?|fp\d+|bf16|fp16|int\d+)\b/g, ' ')
    .replace(/\b\d{6,8}\b/g, ' ')
    .replace(/\b\d{4}\b/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
...
function familyQualityCandidates(modelId, qualityData) {
  const key = modelFamilyKey(modelId);
  if (!key) return [];
  const seen = new Set();
  const candidates = [];
  for (const [candidateKey, entry] of qualityData?.index || []) {
    if (!entry || seen.has(candidateKey)) continue;
    if (modelFamilyKey(candidateKey) !== key) continue;
...
function familyLocalScore(modelId) {
  const key = modelFamilyKey(modelId);
  if (!key) return null;
  // scores.js is the conservative final source of known family members. The
  // resolver does the actual catalog/external precedence; this is only a match
  // candidate for a model that has no direct score.
  const rows = Object.entries(scores)
    .map(([id, score]) => ({ id, score: Number(score) }))
    .filter(row => Number.isFinite(row.score) && row.score > 0 && modelFamilyKey(row.id) === key)
...
export function resolveModelQuality(qualityData, modelId, localScore = null) {
  ...
  // A family match is weaker than an exact curated score and must be reported as
  // such. Prefer a measured catalog member's AA value when one exists; otherwise
  // use its normalized position. This is deliberately after exact local scores.
  const familyEntry = familyQualityCandidates(modelId, qualityData)
    .find(entry => Number.isFinite(Number(entry.aa)) && Number(entry.aa) > 0)
    || familyQualityCandidates(modelId, qualityData)[0];
  if (familyEntry) {
    const aa = Number(familyEntry.aa);
    if (Number.isFinite(aa) && aa > 0) {
      return {
        ...
        isEstimated: true,
        detail: `same-family estimate from ${familyEntry.modelId || 'a catalog model'}; no exact benchmark match`,
        modelId,
      };
    }
    const familyScore = Number(familyEntry.score);
  ...
  const localFamily = familyLocalScore(modelId);
  if (localFamily) {
    const raw = localFamily.score > 1 ? localFamily.score / 100 : localFamily.score;
    const regression = qualityData?.aaFromScoreRegression;
    const aa = aaForEstimatedScore(raw, regression, qualityData?.aaValues);
    return {
      score: scoreForEstimatedAa(aa, raw, qualityData?.aaValues),
      ...
      source: 'family-estimate',
      isEstimated: true,
      detail: `same-family estimate from ${localFamily.id}; no exact benchmark match`,
      modelId,
    };
  }
  // Explicitly total: a model with no evidence still occupies the AA scale at
  // 0.1, rather than being displayed as a missing score or ranked as null.
  const aa = MODEL_QUALITY_FLOOR_AA;
  return {
    // The fallback is specified on the AA scale, so its normalized routing
    // position is exactly 0.001 rather than a percentile-derived approximation.
    // `aa` remains the user-facing AA value of 0.1.
    score: aa / 100,
    aa,
    aaEstimated: true,
    source: 'default-fallback',
    isEstimated: true,
    detail: `no external or exact family score; conservative AA floor ${aa}`,
    modelId,
  };
}
