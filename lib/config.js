 ...
 *    (NVIDIA NIM, Groq, OpenRouter, etc.). Providers are always enabled: the
 ...
 *   }
 *
 * 📖 Multi-account pools, spent in order:
 *   apiKeys values can be string | string[].
 *   Array = multiple credentials, spent in order, with max-turns + 429 backoff.
 *
...
//
// 📖 All three tables are now *derived* from the provider descriptors instead of being
// 📖 hand-maintained here, so a provider's env overrides live with the rest of its
// 📖 definition. The exported behaviour is unchanged — only the source of truth moved.
// 📖 Notable properties the derivation preserves, because they are deliberate:
// 📖   • openai-codex has no API-key env var (its credential is a ChatGPT OAuth refresh
// 📖     token, exchanged for a short-lived access token rather than sent as a bearer).
// 📖   • github-copilot's token is a device-flow credential, so it appears here but not
// 📖     in OAUTH_ACCOUNT_PROVIDERS — it has no account pool.
// 📖   • g4f's key is optional; the relays work without one at lower per-day limits.
// 📖 See lib/providers/catalog.js for the per-provider records behind these.
const ENV_VARS = apiKeyEnvVarTable()

const PROVIDER_BASE_URL_ENV_VARS = baseUrlEnvVarTable()

const PROVIDER_MODEL_ID_ENV_VARS = modelIdEnvVarTable()
 ...
 * @param {string} providerKey — e.g. 'nvidia', 'groq', 'openrouter'
 * @returns {string|null}
 */
...
/** 📖 Providers whose credential is an account sign-in (derived: auth.kind === 'oauth'). */
export const OAUTH_ACCOUNT_PROVIDERS = oauthAccountProviders()

/** 📖 The legacy single-credential field each OAuth provider used before pools (derived). */
const LEGACY_ACCOUNT_SECRET_FIELDS = legacyAccountSecretFieldTable()
