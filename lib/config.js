...
// Every g4f relay is just another base URL onto the same g4f.dev service, so a
// single account key (config `apiKeys.g4f` or `G4F_API_KEY`) covers all of them;
// the user should not have to paste the same secret six times. Credentials stay
// optional: g4f also accepts anonymous traffic once this IP has baked cake
// credits at https://g4f.dev/chat.
const G4F_FAMILY_KEYS = ['g4f', 'g4f-nvidia', 'g4f-groq', 'g4f-gemini', 'g4f-ollama', 'g4f-pollinations'];
function resolveG4fFamilyKey(providerKey) {
  return G4F_FAMILY_KEYS.includes(providerKey) && providerKey !== 'g4f' ? 'g4f' : null;
}
 ...
 * @returns {string|null}
 */
function readConfiguredApiKey(config, providerKey) {
...
export function getApiKey(config, providerKey) {
  const direct = readConfiguredApiKey(config, providerKey)
  if (direct) return direct
  // A g4f relay with no key of its own inherits the shared g4f account key.
  const familyKey = resolveG4fFamilyKey(providerKey)
  return familyKey ? readConfiguredApiKey(config, familyKey) : null
}
 ...
 * @returns {string[]}
 */
function readConfiguredApiKeyPool(config, providerKey) {
  const envVar = ENV_VARS[providerKey] || _legacyOpenAICompatibleEnvVar(providerKey, ENV_VARS)
  if (envVar && process.env[envVar]) {
...
export function getApiKeyPool(config, providerKey) {
  const direct = readConfiguredApiKeyPool(config, providerKey)
  if (direct.length > 0) return direct
  const familyKey = resolveG4fFamilyKey(providerKey)
  return familyKey ? readConfiguredApiKeyPool(config, familyKey) : []
}
...
export function getProviderPingIntervalMs(config, providerKey) {
  ...
  const mins = Number(config?.pingIntervalMinutes)
...
function _emptyConfig() {
  return {
    ...
    // pure Elo-based smartest ranking. minSpeed/minIntell are optional floors.
    selector: {
      enabled: false,
      slope: null,
      minSpeed: null,
...
export function normalizeConfigShape(config) {
  ...
  const pingIntervalMinutes = Number(base.pingIntervalMinutes)
  base.pingIntervalMinutes = Number.isFinite(pingIntervalMinutes) && pingIntervalMinutes >= 1
    ? Math.floor(pingIntervalMinutes)
    : 30
  ...
  if (!base.modelTags || typeof base.modelTags !== 'object' || Array.isArray(base.modelTags)) {
    base.modelTags = {}
  } else {
    const normalizedModelTags = {}
    for (const [modelId, tags] of Object.entries(base.modelTags)) {
      if (typeof modelId !== 'string' || !Array.isArray(tags)) continue
      const normalizedTags = [...new Set(tags
        .map(tag => String(tag || '').trim().toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9_-]/g, '').replace(/^[-_]+|[-_]+$/g, '').slice(0, 32))
        .filter(Boolean))].slice(0, 20)
      if (normalizedTags.length > 0) normalizedModelTags[modelId.trim().toLowerCase()] = normalizedTags
    }
    base.modelTags = normalizedModelTags
  }
