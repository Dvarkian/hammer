...
export function normalizeConfigShape(config) {
  ...
  // Slope-line selector settings: numbers or null; enabled defaults false so the
  // feature only takes over smartest routing after the user interacts with the plot.
  if (!base.selector || typeof base.selector !== 'object' || Array.isArray(base.selector)) {
    base.selector = {}
  }
  const sel = base.selector
  sel.enabled = sel.enabled === true
  const normOptNum = (v) => {
  ...
  if (!sel.enabled) {
    // Keep stored values (so the plot restores sliders after restart) but the
    // selector stays inert unless enabled.
  }
  base.selector = sel
